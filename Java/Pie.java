...
class Pie
{
	...
	public static void main(String args[])throws Exception
	{
        ...
        while (t-->0)
        {
            ...
            int i;
            rad = new long[n];
            for(i=0;i<n;i++) rad[i] = r.nextLong();
            mergeSort(rad, 0, n-1);

            /*double l = 0.001;
            while (l<=pi*rad[n-1]*rad[n-1])
            {
	...
	public static double search(long people)
    {
        double l = 0, h = pi*rad[n-1]*rad[n-1]*1, mid = 0;
        while (h-l>=0.000001)
        {
            mid = l+(h-l)/2;
            if(people>=p(mid)) h = mid;
            else l = mid;
        }
        return l;
    }
	public static long p(double vol)
    {
        long people = 0;
        int i;
        for(i=0;i<n;i++)
        {
            double reqh = vol/(pi*rad[i]*rad[i]);
            long pep = (long)(1.0/reqh);
            people+=pep;
        }
        return people;
    }
